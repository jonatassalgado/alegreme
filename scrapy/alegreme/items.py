...
def get_date(value):
    ...
    if datetimes:
        ...
        parsed_datetimes.append(
            dateparser.parse(datetimes[0], settings={"TIMEZONE": "-0300"})
        )
        return parsed_datetimes

    datetimes = re.search(
        "(\d{2}).+?([A-z]{4,}).+?(\d{4}).+?(\d{2}h\d{0,2}|\d{2})", value
    )
    if datetimes:
        ...
        parsed_datetimes.append(
            dateparser.parse(formatted_datetime, settings={"TIMEZONE": "-0300"})
        )
        return parsed_datetimes
...
class Event(scrapy.Item):
    ...
    description = scrapy.Field(
        input_processor=MapCompose(clean_description), output_processor=TakeFirst()
    )
    ...
    latitude = scrapy.Field(
        input_processor=MapCompose(get_event_latitude), output_processor=TakeFirst()
    )
    longitude = scrapy.Field(
        input_processor=MapCompose(get_event_longitude), output_processor=TakeFirst()
    )
...
class Movie(scrapy.Item):
    ...
    genres = scrapy.Field(
        input_processor=MapCompose(get_movie_genres), output_processor=Identity()
    )
    year = scrapy.Field(
        input_processor=MapCompose(get_movie_year), output_processor=TakeFirst()
    )
    rating = scrapy.Field(output_processor=TakeFirst())
    cast = scrapy.Field(output_processor=Identity())
    age_rating = scrapy.Field(output_processor=TakeFirst())
    screenings = scrapy.Field(input_processor=Identity())
...
class MoviePlace(scrapy.Item):
    google_id = scrapy.Field(
        input_processor=MapCompose(get_movie_google_id), output_processor=TakeFirst()
    )
    name = scrapy.Field(output_processor=TakeFirst())
    google_maps = scrapy.Field(output_processor=TakeFirst())
    languages = scrapy.Field(input_processor=Identity())
...
class MovieLanguage(scrapy.Item):
    name = scrapy.Field(output_processor=TakeFirst())
    screen_type = scrapy.Field(output_processor=TakeFirst())
    times = scrapy.Field()
...
class Poster(scrapy.Item):
    shortcode = scrapy.Field(output_processor=TakeFirst())
    media = scrapy.Field(input_processor=Identity())
    caption = scrapy.Field(output_processor=TakeFirst())
    taken_at_timestamp = scrapy.Field(output_processor=TakeFirst())
    source_url = scrapy.Field(
        input_processor=MapCompose(remove_url_params), output_processor=TakeFirst()
    )
    location = scrapy.Field(output_processor=TakeFirst())
    owner = scrapy.Field(input_processor=Identity())


class PosterMedia(scrapy.Item):
    is_video = scrapy.Field(output_processor=TakeFirst())
    display_url = scrapy.Field(output_processor=TakeFirst())
