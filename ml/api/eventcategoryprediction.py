...
class EventCategoryPrediction(object):
    ...
    def __init__(self):
        if os.environ['IS_DOCKER'] == 'true':
            ...
            last_file = max(filter(regex.search, os.listdir(
                '/var/www/scrapy/data/classified/')))
    ...
    def __cleanning_text(self, text):
        ...
        # remove numbers
        text = re.sub(
            " \d+|\d{2}:\d{2}|\d{2}/\d{2}|\dº|\(\d+\)|\d{2}h\d{2}|\d{2}.|\d{2}h|\d\d|\d{2}min|,|º", "", text)
    ...
    def clean(self):
        self.base = self.base.loc[(self.base['description'].notna()) & (
            self.base['label_b'].notna())]
        self.X = self.base['name'].str.cat(
    ...
    def train(self):
        X_train, X_test, y_train, y_test = train_test_split(
            self.X, self.y, test_size=0.15, random_state=True)
        ...
        for prediction in predictions:
            ...
            best_two_labels_index = hq.nlargest(
                2, range(len(prediction)), prediction.take)
            predictions_labels.append(classificator.classes_[
                                      best_two_labels_index])
