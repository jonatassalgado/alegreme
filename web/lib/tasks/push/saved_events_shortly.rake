...
namespace :push do
	desc 'Envia push para usuários avisando dos eventos que ocorrem em 2 horas'
	task saved_events_shortly: :environment do

		puts "Task push:saved_events_shortly iniciada em #{DateTime.now}}".white
		...
		events_occuring_shortly_ids  = Event.select("id").in_next_2_hours.map(&:id)

		users_with_saved_events.each do |user|
			if user.saved_events_ids.any? { |saved| events_occuring_shortly_ids.include?(saved) }
		...
		users_to_notify.each do |user|
			user.notifications_devices.each do |device|
				...
				notification.data             = {
						...
						message: "Você tem eventos salvos que acontecem nas próximas horas.",
				...
				notification.time_to_live = 7200
		...
		puts "Task push:saved_events_shortly finalizada em #{DateTime.now}".white
