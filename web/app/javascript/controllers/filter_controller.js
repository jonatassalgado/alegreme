...
export default class FilterController extends ApplicationController {
	...
	select(event) {
		if (this.MDCChipSet.selectedChipIds.length > 0) {
		...
		// {
		// 	categories: this.data.get('categoriesFilters'),
		// 	days:       this.data.get('ocurrencesFilters'),
		// 	kinds:      this.data.get('kindsFilters')
		// })
	...
	// 		chipEl.dataset.target = "filter.chip";
	// 		chipEl.dataset.action = "click->filter#select";
	//
	...
	// 	          <path class="mdc-chip__checkmark-path" fill="none" stroke="black" d="M1.73,12.91 8.1,19.28
	// 22.79,4.59" /> </svg> </div> <div class="mdc-chip__text"> ${text} </div> `; const currentMDCChipPromise = new
	// Promise((resolve, reject) => { const currentMDCChip = this.MDCChipSet.chips.filter(chip => chip.id ==
	// chipEl.id);  resolve(currentMDCChip[0]); });  currentMDCChipPromise .then((currentMDCChip) => {
	// currentMDCChip.selected = true; render(chipTemplate(this.inputTarget.value.toLowerCase()), chipEl)
	// this.inputTarget.value = ''; }) .catch((err) => { console.log(err); }); }
