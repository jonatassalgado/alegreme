class FeedsController < ApplicationController
  ...
  def train
    @events = {
      # for_me: Event.joins(:calendars).order('day_time ASC').uniq,
      underground: Event.where("(personas -> 'primary' ->> 'name') = 'underground' AND (ocurrences -> 'dates' ->> 0)::timestamptz > ?", DateTime.now).order(Arel.sql("(ocurrences -> 'dates' ->> 0) ASC")).uniq,
      praieiro: Event.where("(personas -> 'primary' ->> 'name') = 'praieiro' AND (ocurrences -> 'dates' ->> 0)::timestamptz > ?", DateTime.now).order(Arel.sql("(ocurrences -> 'dates' ->> 0) ASC")).uniq,
      cult: Event.where("(personas -> 'primary' ->> 'name') = 'cult' AND (ocurrences -> 'dates' ->> 0)::timestamptz > ?", DateTime.now).order(Arel.sql("(ocurrences -> 'dates' ->> 0) ASC")).uniq,
      aventureiro: Event.where("(personas -> 'primary' ->> 'name') = 'aventureiro' AND (ocurrences -> 'dates' ->> 0)::timestamptz > ?", DateTime.now).order(Arel.sql("(ocurrences -> 'dates' ->> 0) ASC")).uniq,
      geek: Event.where("(personas -> 'primary' ->> 'name') = 'geek' AND (ocurrences -> 'dates' ->> 0)::timestamptz > ?", DateTime.now).order(Arel.sql("(ocurrences -> 'dates' ->> 0) ASC")).uniq,
      hipster: Event.where("(personas -> 'primary' ->> 'name') = 'hipster' AND (ocurrences -> 'dates' ->> 0)::timestamptz > ?", DateTime.now).order(Arel.sql("(ocurrences -> 'dates' ->> 0) ASC")).uniq,
      turista: Event.where("(personas -> 'primary' ->> 'name') = 'turista' AND (ocurrences -> 'dates' ->> 0)::timestamptz > ?", DateTime.now).order(Arel.sql("(ocurrences -> 'dates' ->> 0) ASC")).uniq,
      zeen: Event.where("(personas -> 'primary' ->> 'name') = 'zeen' AND (ocurrences -> 'dates' ->> 0)::timestamptz > ?", DateTime.now).order(Arel.sql("(ocurrences -> 'dates' ->> 0) ASC")).uniq
    }
