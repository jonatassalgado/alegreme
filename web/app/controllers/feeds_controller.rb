...
class FeedsController < ApplicationController
	...
	def index
		...
		@events_this_week             ||= Event.not_ml_data.active.in_categories(Event::CATEGORIES, {not_in: %w(anúncio slam curso protesto outlier)}).with_high_score.not_in_saved(current_user).in_days((DateTime.now.beginning_of_day.yday..(DateTime.now.beginning_of_day.yday + 8))).includes(:place).order_by_date
