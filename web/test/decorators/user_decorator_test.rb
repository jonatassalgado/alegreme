...
class UserDecoratorTest < ActiveSupport::TestCase
	...
	test "events_from_followed_features" do
		context 'exist 2 events from features followed' do |context|
			...
			assert_equal 2, user.events_from_followed_features.count, "#{context} -> should return all events with same features liked"
		end
		...
		context 'exist 1 event from features followed' do |context|
			...
			assert_equal 1, user.events_from_followed_features.count, "#{context} -> should return all events with same features liked"
		end
	end
end